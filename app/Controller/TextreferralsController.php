 ...
 * @property Text $Text
 */
class TextreferralsController extends AppController {
 ...
 */
	...
	$this->layout = 'admin';
		$this->Textreferral->recursive = 0;
		$this->paginate = array(
      		'limit' => 10
  		);
  		
  		$pending = $this->Textreferral->find('all');

    	$this->set('pending', $this->paginate('Textreferral'));
 ...
 */
	...
	$this->layout = 'admin';
		$this->Textreferral->id = $id;
		if (!$this->Textreferral->exists()) {
			throw new NotFoundException(__('Invalid text'));
		}
		$this->set('text', $this->Textreferral->read(null, $id));
	}
 ...
 */
	...
	$this->layout = 'admin';
		if ($this->request->is('post')) {
			$this->Textreferral->create();
			if ($this->Textreferral->save($this->request->data)) {
				...
				$this->Textreferral->set('enabled', 1);
				$this->Textreferral->save();
 ...
 */
	...
	$this->layout = 'admin';
		...
		if ($this->request->is('post') || $this->request->is('put')) {
			...
			$this->request->data = $this->Textreferral->read(null, $id);
		}
	}
	...
	$this->layout = 'admin';
		...
		$this->Textreferral->set('enabled', 0);
				$this->Textreferral->save();
 ...
 */
	...
	$this->layout = 'admin';
		...
		if ($this->request->is('post') || $this->request->is('put')) {
			...
			$text = $this->Textreferral->findById($id);
...
//				if (!$reply) {echo "Error: Cannot send Text on Avid. Please contact support."; exit;}
    		...
    		foreach ($users as $user) {
    			...
    			$this->Text->set('content', $text['Textreferral']['content']);
    			$this->Text->set('scheduledon', $text['Textreferral']['scheduledon']);
    			$this->Text->set('admin', 1);
    			$this->Text->set('originator', $id);
    			$this->Text->save();
    		}
    		
    		$this->Textreferral->set('sent', 1);
    		$this->Textreferral->save();
 ...
 */
	...
	$this->layout = 'admin';
		if (!$this->request->is('post')) {
			...
			$this->loadModel('Textreferral');
			$this->Textreferral->deleteAll('1 = 1');		
